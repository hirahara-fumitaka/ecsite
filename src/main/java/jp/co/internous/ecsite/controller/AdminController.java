...
//	forwardで返すurlは/ecsite/admin/welcomeだが実際に行くのは/ecsite/admin/
	@PostMapping("/welcome")
	public String welcome(LoginForm form, Model model) {
		
		MstUser user = userMapper.findByUserNameAndPasswod(form);
		

		if (user == null) {
			model.addAttribute("errMessage","ユーザー名またはパスワード名が違います。");
